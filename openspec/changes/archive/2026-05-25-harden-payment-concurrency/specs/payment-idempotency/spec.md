...
- **THEN** apenas um pagamento DEVE ser criado
- **AND** ambas as requisicoes DEVEM retornar `201 Created` com o mesmo `paymentId`
...
- **WHEN** ambas forem processadas concorrentemente
- **THEN** apenas uma DEVE ser processada com sucesso ou rejeitada por limite
- **AND** a outra DEVE retornar `409 Conflict`

### Requirement: Registro de chave idempotente
O sistema DEVE registrar a chave idempotente dentro da transacao do pagamento, utilizando `INSERT ... ON CONFLICT DO NOTHING` para atomicidade. O registro DEVE armazenar o request hash, o paymentId, o response status e o response body para permitir retorno do resultado original em retries.
...
- **THEN** o sistema DEVE registrar em `payment_idempotency_keys` dentro da mesma transacao do pagamento
- **AND** o registro DEVE conter a chave, o request hash, o `paymentId`, o status `201` e o body da resposta
...
- **THEN** o sistema DEVE registrar em `payment_idempotency_keys` dentro da mesma transacao
- **AND** o registro DEVE conter a chave, o request hash e o status `422`
