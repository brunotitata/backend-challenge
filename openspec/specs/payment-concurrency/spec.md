...
O sistema MUST garantir que acessos simultaneos ao endpoint `POST /wallets/{walletId}/payments` para a mesma carteira nao ultrapassem os limites definidos pela politica ativa. A verificacao de limite e a gravacao do consumo MUSTM ser atomicas.
...
- **THEN** o sistema MUST aprovar apenas um pagamento
- **AND** o outro MUST retornar `422 Unprocessable Entity`
...
- **THEN** o sistema MUST aprovar apenas o primeiro pagamento
- **AND** o segundo MUST retornar `422 Unprocessable Entity`
...
- **THEN** ambos os pagamentos MUSTM ser aprovados com `201 Created`

### Requirement: Insercao atomica da linha de consumo
O sistema MUST garantir que a linha em `limit_consumptions` seja inserida atomicamente antes do lock, prevenindo que duas requisicoes concorrentes vejam `consumed_amount = 0` simultaneamente.
...
- **THEN** apenas uma MUST criar a linha com `consumed_amount = 0`
- **AND** a segunda MUST reutilizar a linha existente com o lock adquirido
