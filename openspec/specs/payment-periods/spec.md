...
O sistema DEVE classificar como `DAYTIME` qualquer `occurredAt` que, no timezone operacional (`America/Sao_Paulo`), ocorra em um dia util (segunda a sexta) entre `06:00:00` (inclusive) e `18:00:00` (exclusive).
...
- **THEN** o `periodType` DEVE ser `DAYTIME`
...
O sistema DEVE classificar como `NIGHTTIME` qualquer `occurredAt` que, no timezone operacional, ocorra em um dia util (segunda a sexta) entre `18:00:00` (inclusive) e `06:00:00` do dia seguinte (exclusive).
...
- **WHEN** o `PeriodClassifier.classify()` for chamado
- **THEN** o `periodType` DEVE ser `NIGHTTIME`
...
O sistema DEVE classificar como `WEEKEND` qualquer `occurredAt` que, no timezone operacional, ocorra em um sabado ou domingo, independentemente do horario. Final de semana tem prioridade sobre horario.

#### Scenario: Sabado as 00:00:00 e WEEKEND
- **GIVEN** um `occurredAt` em um sabado as `00:00:00` no timezone `America/Sao_Paulo`
- **WHEN** o `PeriodClassifier.classify()` for chamado
- **THEN** o `periodType` DEVE ser `WEEKEND`
...
O `periodStart` para `DAYTIME` DEVE ser o mesmo dia local as `06:00:00`.
...
- **WHEN** o `PeriodClassifier.classify()` for chamado
- **THEN** o `periodStart` DEVE ser o mesmo dia as `06:00:00` no mesmo timezone

### Requirement: Calcular periodStart para WEEKEND
O `periodStart` para `WEEKEND` DEVE ser o mesmo dia local as `00:00:00`.
...
- **WHEN** o `PeriodClassifier.classify()` for chamado
- **THEN** o `periodStart` DEVE ser o mesmo dia as `00:00:00` no mesmo timezone

### Requirement: Calcular periodStart para NIGHTTIME apos 18:00
Para horario noturno a partir de `18:00:00`, o `periodStart` DEVE ser o mesmo dia as `18:00:00`.
...
- **WHEN** o `PeriodClassifier.classify()` for chamado
- **THEN** o `periodType` DEVE ser `NIGHTTIME`
- **AND** o `periodStart` DEVE ser a segunda-feira as `18:00:00`

### Requirement: Calcular periodStart para NIGHTTIME antes de 06:00
Para horario noturno antes de `06:00:00`, o `periodStart` DEVE ser o dia anterior as `18:00:00`.
...
Segunda `23:00` e terca `01:00` DEVEM ter o mesmo `periodStart` (ambos pertencem a mesma noite iniciada segunda as `18:00`).
...
- **THEN** ambos DEVEM ter `periodType` igual a `NIGHTTIME`
- **AND** ambos DEVEM ter o mesmo `periodStart` (segunda-feira as `18:00:00`)

### Requirement: Classificacao independe de framework e banco
O `PeriodClassifier` DEVE ser uma funcao pura que recebe `Instant` e `ZoneId` e retorna `PeriodClassification`. Nao DEVE depender de Ktor, jOOQ, banco de dados ou qualquer framework externo.
...
- **THEN** `classify()` DEVE funcionar sem banco, sem HTTP e sem configuracao externa
