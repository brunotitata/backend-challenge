...
- **THEN** o sistema MUST retornar `201 Created` com `id` em formato UUID, `ownerName` igual a `Ana Silva` e `createdAt` em ISO-8601 UTC
...
- **THEN** o sistema MUST retornar `400 Bad Request` usando a estrutura padrao de erro
...
- **THEN** o sistema MUST gravar a carteira na tabela `wallets` com `id`, `owner_name` e `created_at`
...
- **WHEN** uma carteira valida e criada com sucesso
- **THEN** o sistema MUST registrar um vinculo ativo entre a carteira e a politica `DEFAULT_VALUE_LIMIT`
...
- **THEN** o sistema MUST resolver a politica `DEFAULT_VALUE_LIMIT` para essa carteira
...
- **WHEN** uma carteira valida e criada com sucesso
- **THEN** o sistema MUST registrar apenas um vinculo ativo para a carteira criada
