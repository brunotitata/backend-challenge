...
O sistema DEVE expor `GET /wallets/{walletId}/payments` para listar pagamentos de uma carteira com paginação por cursor, filtro opcional por data e ordenação estável.
...
- **AND** o body DEVE conter `data` com os 3 pagamentos
- **AND** o body DEVE conter `meta` com `nextCursor`, `previousCursor`, `total` e `totalMatches`
...
- **THEN** o sistema DEVE retornar apenas pagamentos a partir de 2024-08-26

#### Scenario: Filtra por endDate
- **GIVEN** pagamentos com `occurredAt` em 2024-08-25 e 2024-08-26
- **WHEN** `endDate=2024-08-25T23:59:59.9999Z` for enviado
- **THEN** o sistema DEVE retornar apenas pagamentos ate 2024-08-25
...
- **THEN** o sistema DEVE retornar apenas pagamentos de 2024-08-25
...
- **THEN** o sistema DEVE retornar 20 pagamentos na pagina atual
- **AND** `meta.nextCursor` DEVE ser uma string nao nula
...
- **THEN** o sistema DEVE retornar os 5 pagamentos restantes

#### Scenario: previousCursor permite navegacao reversa
- **GIVEN** uma carteira com 25 pagamentos aprovados
- **WHEN** a segunda pagina for obtida via cursor
- **THEN** `meta.previousCursor` DEVE ser uma string nao nula
- **AND** ao usar `previousCursor`, o sistema DEVE retornar a pagina anterior
...
- **THEN** a ordem DEVE ser deterministica por `occurredAt` ASC e `id` ASC
...
- **THEN** o sistema DEVE retornar apenas pagamentos da primeira carteira

#### Scenario: Lista apenas pagamentos APPROVED
- **GIVEN** uma carteira com pagamentos `APPROVED` e `REJECTED`
- **WHEN** a listagem for solicitada
- **THEN** o sistema DEVE retornar apenas pagamentos com `status: "APPROVED"`

### Requirement: Validar parametros de consulta
O sistema DEVE validar os parametros da query e retornar erros apropriados.
...
A resposta DEVE seguir o formato `data` e `meta` conforme o README do desafio.
...
- **THEN** cada item em `data` DEVE conter `id`, `walletId`, `amount`, `occurredAt`, `status`, `createdAt`, `updatedAt`

#### Scenario: Resposta contem meta com todos os campos
- **GIVEN** uma lista com pagamentos aprovados
- **WHEN** a requisicao for bem-sucedida
- **THEN** `meta` DEVE conter `nextCursor`, `previousCursor`, `total` e `totalMatches`

#### Scenario: totalMatches retorna null
- **GIVEN** uma lista com pagamentos aprovados
- **WHEN** a requisicao for bem-sucedida
- **THEN** `meta.totalMatches` DEVE ser `null`

#### Scenario: total reflete contagem total da carteira
- **GIVEN** uma carteira com 10 pagamentos aprovados
- **WHEN** a requisicao for bem-sucedida
- **THEN** `meta.total` DEVE ser `10`
